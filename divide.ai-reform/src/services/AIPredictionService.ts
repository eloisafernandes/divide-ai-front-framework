...
export async function getPredictionByUserAndGroupId(groupId: number): Promise<ChatResponse | null> {
  try {
    ...
    const response = await api.get<ApiResponse<ChatResponse>>(`/reform/chat-completion/user/${id}/group/${groupId}`, {
...
export async function createChat(request: IAIPredictionRequest): Promise<IAIPrediction | null> {
  try {
    const token = getUserLocalStorage()?.token;
    const response = await api.post<ApiResponse<IAIPrediction>>('/reform/chat-completion', request, {
      headers: { 'Authorization': `Bearer ${token}` }
    });
